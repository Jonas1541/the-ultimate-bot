...
// Estratégias Quebradas (Decoupled)
#include <TheUltimateBot/Operations/Trend/TrendStrategy.mqh>
#include <TheUltimateBot/Operations/Volatility/BreakoutStrategy.mqh>

// --- INPUTS: INTERFACE GRÁFICA (NOVO) ---
input group "Interface Gráfica" input double Inp_UIScale =
    1.2; // Fator de Escala (1.0 = Padrão, 1.3 = Grande)

// --- INPUTS: GERENCIAMENTO DE RISCO ---
input group "Gerenciamento de Risco" input double Inp_RiskPercent =
    1.0; // Risco por Trade (% do Saldo)

// --- INPUTS: SEGURANÇA GLOBAL ---
input group "Segurança Global" input double Inp_DailyLoss =
    50.0; // Limite de Perda Diária ($)

// --- INPUTS: HORÁRIO ---
input group "Horário de Negociação" input int Inp_StartHour = 9; // Hora Início
...
int OnInit() {
  ...
  TimeFilter *timeService =
      new TimeFilter(Inp_StartHour, Inp_StartMin, Inp_EndHour, Inp_EndMin);
  ...
  DailyLossValidator *lossValidator =
      new DailyLossValidator(session, Inp_DailyLoss);
  ...
  // A. Trend Strategy
  TrendStrategy *trendStrat = new TrendStrategy(_Symbol);
  trendStrat.AddValidator(timeValidator);
  trendStrat.AddValidator(lossValidator);

  // B. Breakout Strategy
  BreakoutStrategy *breakStrat = new BreakoutStrategy(_Symbol);
  breakStrat.AddValidator(timeValidator);
  breakStrat.AddValidator(lossValidator);

  // ---------------------------------------------------------
  // 5. REGISTRO NO CÉREBRO (Competition Logic)
  // ---------------------------------------------------------
  g_kernel.GetManager().Register(trendStrat);
  g_kernel.GetManager().Register(
      breakStrat); // Breakout tem score maior (85 vs 65)
  ...
  PrintFormat(">>> [BOOT] Config: Risk=%.1f%% | UI Scale=%.1f | Strategies: "
              "Trend+Breakout",
...
void OnDeinit(const int reason) {
  ...
  if (CheckPointer(g_kernel) == POINTER_DYNAMIC)
    delete g_kernel;
...
void OnTick() {
  ...
  if (g_dashboard != NULL) {
    ...
    if (PositionSelect(_Symbol)) {
      ...
      g_dashboard.Log("Monitorando Mercado (Adaptive)...");
    }
    ...
    if (ObjectGetInteger(0, "TheUltimateBot_Btn_Panic", OBJPROP_STATE) == 1) {
      ...
      if (g_dashboard != NULL)
        g_dashboard.Log("!!! PÂNICO (VIA POLLING) !!!");
...
void OnChartEvent(const int id, const long &lparam, const double &dparam,
                  const string &sparam) {
  // DEBUG BRUTO: Imprimir qualquer evento que não seja MouseMove (pra não
  // floodar)
  if (id != CHARTEVENT_MOUSE_MOVE) {
    PrintFormat(">>> [EVENT] ID=%d | LParam=%d | DParam=%.2f | SParam='%s'", id,
  ...
  if (id == CHARTEVENT_OBJECT_CLICK) {
    ...
    if (StringFind(sparam, "Panic") >= 0) {
      ...
      if (g_dashboard != NULL)
        g_dashboard.Log("!!! PÂNICO CONFIRMADO !!!");
